...
默认**关闭**，观众页看不到打赏入口。开启不用改代码、不用重新部署，把收款码图片传到服务器上即可：

```bash
scp 收款码.png ubuntu@服务器IP:~/song-request/data/tip.png
```

传完即生效（观众下次打开或刷新页面就会看到）。观众页底部会出现一张「觉得好听？请歌手喝杯东西」的卡片，长按识别或截图扫码，钱直接进收款码对应的个人账户，不经过这套系统。

- 图片建议正方形 PNG；微信和支付宝的收款码可以拼在一张图里
- 图片放在 `data/` 下：不进 git 仓库，`deploy.sh` 重新发布代码也不会覆盖它
- 想关掉就删掉服务器上的文件：`ssh ubuntu@服务器IP 'rm ~/song-request/data/tip.png'`
- 收款码是个人收款码，别拿来做商业经营收款（平台风控会冻卡）；路演打赏这种小额个人往来一般没问题
